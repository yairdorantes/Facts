...
const Reaction = ({ fact, reaction }) => {
  const { openReactions, handleReactionVisibility } = useBearStore();
  const [visibleReacts, setVisibleReacts] = useState(openReactions);
  ...
  const bind = useLongPress(
    () => {
      handleReactionVisibility(!openReactions);
    },
    {
      onCancel: () => {
        if (!btnIcon) {
  ...
  // useEffect(() => {
  //   console.log("xx");
  //   console.log(openReactions);
  // }, []);
  return (
    <div className="topota">
      ...
      {user && btnIcon ? (
        <div data-reaction="true" className="flex flex-col">
          <button
            data-reaction="true"
            {...bind()}
